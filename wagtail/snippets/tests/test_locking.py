...
class TestEditLockedSnippet(BaseLockingTestCase):
    ...
    def test_edit_get_locked_by_self(self):
        ...
        for permissions in cases:
            with self.subTest(
                ...
                self.assertTagInHTML(
                    f'<button type="button" data-action="w-action#post" data-controller="w-action" data-w-action-redirect-value="false" data-w-action-url-value="{unlock_url}">Unlock</button>',
                    html,
                    count=2,
                    allow_extra_attrs=True,
                )
    ...
    def test_edit_get_unlocked_no_lock_permission(self):
        ...
        self.assertTagInHTML(
            f'<button type="button" data-action="w-action#post" data-controller="w-action" data-w-action-redirect-value="false" data-w-action-url-value="{lock_url}">Lock</button>',
            html,
            count=0,
            allow_extra_attrs=True,
        )
