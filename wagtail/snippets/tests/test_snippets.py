...
class TestScheduledForPublishLock(BaseTestSnippetEditView):
    ...
    def test_edit_get_scheduled_for_publishing_with_publish_permission(self):
        ...
        self.assertTagInHTML(
            f'<button data-action="w-action#post" data-controller="w-action" data-w-action-redirect-value="false" data-w-action-url-value="{unschedule_url}">Cancel scheduled publish</button>',
            html,
            count=1,
            allow_extra_attrs=True,
        )
